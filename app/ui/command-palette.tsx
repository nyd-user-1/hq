...
// The Claude "spark" symbol (Wikimedia: Claude_AI_symbol.svg) — a FILLED path at
// viewBox 100, not a stroked lucide glyph, so it has its own svg. fill=currentColor
// so it inherits the row's kind color (Claude clay for docs, via kindIconColor).
const IconClaude = () => (
  <svg width="15" height="15" viewBox="0 0 100 100" fill="currentColor" aria-hidden>
          ...
          {viewing ? (
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-3">
              <div className="flex items-center gap-3 border-b border-dashed border-zinc-800 pb-3">
                ...
                >
                  {viewing.kind === "doc" ? "Claude Doc" : viewing.kind}
                </span>
              </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              {isHit && cmd.kind && (
                                ...
                                >
                                  {cmd.kind === "doc" ? "Claude Doc" : cmd.kind}
                                </span>
                              )}
                            </button>
                          );
