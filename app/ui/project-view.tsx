...
// PROJECTS browser — a 1:1 port of the Files view (same shell + the same Finder
// table) that is "Files + sessions": it lists every file Claude wrote to disk AND
// the rich session rows (from /api/sessions/all), merged into one sortable table.
// Sessions map onto the FileRow shape (kind "session"), so openHref already opens
// them in the reader and the table styles them like any other row. Lives in the
// center column as an overlay (project-overlay.tsx); the terminal stays underneath.
...
// Open a row in the /search panel reader — identical to Files (keeps ?center so the
// table stays put and the reader opens in the RIGHT panel; carries the pins).
...
export default function ProjectView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Rich session rows mapped onto FileRow; they WIN over any transcript file row
      // with the same ref (so a session shows once, with its real title).
  ...
  return (
    <div className={`flex min-h-0 flex-1 flex-col gap-3 font-mono ${focusMode ? "mx-auto w-full max-w-3xl" : ""}`}>
      ...
      {/* filter */}
      <SearchField value={q} onChange={setQ} placeholder="Filter files & sessions by name or path…" className="mt-3 max-w-md" />

      {/* table — the Files Finder table, now carrying files + sessions */}
