      ...
      <Boundary label="skills-panel.tsx">
        ...
        <footer className="shrink-0 border-t border-dashed border-zinc-800 pt-3 font-mono text-[10px] leading-relaxed text-zinc-600">
          {selected
            ...
            : `${skills.length} skills · yours and plugin-shipped. Click one to read it.`}
        </footer>
      </Boundary>
    </AppPanel>
  );
...
// The right-edge meta: the SKILL.md token estimate.
...
function SkillCard({ s, onOpen }: { s: LibrarySkill; onOpen: (s: LibrarySkill) => void }) {
  const dot = s.source === "user" ? "text-blue-500" : "text-emerald-500";
  return (
    <div
      role="button"
      tabIndex={0}
    ...
    >
      ...
      {s.description && (
        <p className="mt-3 line-clamp-3 text-[11px] leading-snug text-zinc-500">{s.description}</p>
      )}
    </div>
  );
}
