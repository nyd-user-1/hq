...
export const DEFAULT_POLICY = {
  ...
  allowBashPatterns: [
    ...
    // NB: awk and sed are deliberately NOT here. Both execute code / write files
    // WITHOUT any shell metacharacter (`awk -f prog`, `awk 'BEGIN{system(...)}'`,
    // `sed -n 's/a/b/w /path'`, `sed -i`), so they'd slip past the metachar guard
    // in isUnsafeBash and auto-allow a write/exec. Reading is already covered by
    // cat/head/tail/grep; the convenience isn't worth the escalation.
    "^(grep|rg|ag|ack|sort|uniq|cut)\\b",
  ],
  denyTools: [],
  denyBashPatterns: [],
};
...
function policyDir() {
  // Respect CLAUDE_CONFIG_DIR (mirrors lib/config.ts claudeHome).
  return path.join(process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), ".claude"), "hq");
}
...
export function isUnsafeBash(cmd) {
  ...
  // awk / sed execute code or write files with NO shell metacharacter (`awk -f`,
  // `awk 'BEGIN{system()}'`, `sed -n 's//w file'`, `sed -i`/`-f`), so the metachar
  // guard above can't catch them — escalate unconditionally (they're also removed
  // from the read-only allow-list). A benign `sed -n '5p'` just gets an ask.
  if (/^\s*(awk|sed)\b/.test(cmd)) return true;
