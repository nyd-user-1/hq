...
export const COMPONENTS: ComponentEntry[] = [
  ...
  { name: "FilterChip", file: "app/ui/filter-chip.tsx", kind: "presentational", status: "approved", desc: "Filter chip — rounded {label · count} toggle; one component for every facet (source/category/scope), optional truncate. Replaced the identical SrcChip/CatChip/ScopeChip copies across the 8 console members. ✅" },
  { name: "Switch", file: "app/ui/switch.tsx", kind: "presentational", status: "approved", desc: "The one on/off toggle — emerald when on, role=switch. hq's single switch UI; no second switch component. ✅" },
  { name: "GroupSwitchChip", file: "app/ui/group-switch-chip.tsx", kind: "presentational", status: "approved", desc: "The one panel-group switcher — file-path chip revealing a group's members on hover; row TEXT swaps in place, ↗ pops out. Generic over the group; optional extraRows + sortAZ. Replaced the 6 per-group switch-chips. ✅" },
  { name: "GroupNavChips", file: "app/ui/group-nav-chips.tsx", kind: "presentational", status: "approved", desc: "The one panel-group ‹ › pair — steps the active member back/forward (wrapping). Generic over the group. Replaced the 6 per-group nav-chips. ✅" },
  ...
  { name: "Button", file: "app/ui/button.tsx", kind: "presentational", status: "approved", desc: "The standard action button — plain (borderless card action: Install / card Run) + outline (bordered detail-header action: Run / Apply / Copy) variants; label swaps on state at the call site; spreads native button props. ✅" },
