...
export type Shape =
  ...
  | { kind: "stackedArea"; title: string; dayLabels: string[]; series: { name: string; tone?: Tone; points: number[] }[]; capL: string; capR: string; range?: boolean }
...
const REGISTRY: Compute[] = [
  ...
  { def: { id: "tokens_stacked_area", label: "Tokens by model (area)", group: "Tokens", kind: "stackedArea", scopes: ["fleet"] }, fleet: () => { const { dayLabels, series } = tokensByDayByTier(90); const tones: Record<string, Tone> = { Opus: "orange", Sonnet: "blue", Haiku: "green", Fable: "red", Mythos: "amber", Other: "zinc" }; return { kind: "stackedArea", title: "Tokens by model", range: true, dayLabels, capL: dayLabels[0] ?? "", capR: "today", series: series.map((s) => ({ name: s.tier, tone: tones[s.tier] ?? "zinc", points: s.points })) }; } },
  ...
  { def: { id: "tokens_by_session_area", label: "Tokens by session (area)", group: "Tokens", kind: "stackedArea", scopes: ["fleet"] }, fleet: () => { const { dayLabels, byFile } = perFileDayWeights(90); const palette: Tone[] = ["blue", "orange", "green", "amber", "red", "zinc"]; const top = [...byFile.entries()].map(([file, points]) => ({ id: path.basename(file, ".jsonl"), points, total: points.reduce((a, b) => a + b, 0) })).sort((a, b) => b.total - a.total).slice(0, 5); return { kind: "stackedArea", title: "Tokens by session", range: true, dayLabels, capL: dayLabels[0] ?? "", capR: "today", series: top.map((e, i) => ({ name: e.id.slice(0, 8), tone: palette[i % palette.length], points: e.points })) }; } },
