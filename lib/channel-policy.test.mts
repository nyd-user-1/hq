// Classifier unit test for lib/channel-policy.ts. HOME-isolated: we point HOME at
// a fresh temp dir BEFORE importing the module, so reading/seeding the policy file
// touches the sandbox, never the real ~/.claude/hq/channel-policy.json.
//
// Run:  node --experimental-strip-types --test lib/channel-policy.test.mts
...
const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), "hq-channel-policy-"));
