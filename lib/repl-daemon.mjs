...
// Respect CLAUDE_CONFIG_DIR (mirrors lib/config.ts claudeHome) so a relocated
// config dir still lands the daemon's sidecars alongside the CLI's own state.
const HQ_DIR = path.join(process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), ".claude"), "hq");
...
function spawnRepl(key, opts) {
  ...
  // Spawn with a CLEANED env, not the daemon's own (issue #2). The launchd dev
  // server leaks two things into every hq-driven session: NODE_ENV=development
  // (so the session's own `next build` hits the non-standard-NODE_ENV trap) and a
  // truncated PATH (so /opt/homebrew/bin tools like `gh` read as "not installed").
  // Strip NODE_ENV and restore a sane PATH so a driven session behaves like a
  // normal login shell.
  //
  // The THIRD leak, and the nastiest: when HQ runs as the packaged desktop
  // standalone, its env carries Next's private config (__NEXT_PRIVATE_STANDALONE_CONFIG,
  // __NEXT_PRIVATE_ORIGIN). Next reads that var INSTEAD of next.config.ts, so any
  // `next build`/`next dev` run inside a driven session silently inherits HQ's own
  // config — JSON-serialized, which strips every function-valued option. The build
  // then dies on `generate is not a function` (generateBuildId is gone) and dev on
  // Turbopack's "Invalid distDirRoot", both pointing at a config the user never
  // wrote. lib/dev-server.ts already filters these for spawned dev servers; a
  // driven Claude session needs the same clean slate.
  const childEnv = { ...process.env, HQ_DAEMON_SOCK: SOCK, HQ_REPL_SESSION: key };
  delete childEnv.NODE_ENV;
  for (const k of Object.keys(childEnv)) {
    if (k === "HQ_BUILD_DIR" || k.startsWith("__NEXT") || k.startsWith("NEXT_PRIVATE"))
      delete childEnv[k];
  }
  childEnv.PATH = `${process.env.PATH ?? ""}:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin`;
