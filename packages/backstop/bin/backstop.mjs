...
//
//   npx @nysgpt/backstop            install + start
//   npx @nysgpt/backstop eject      remove everything, restore direct API
//   npx @nysgpt/backstop status     report what is installed
//   npx @nysgpt/backstop doctor     check every invariant that, if broken,
//                                   takes the account down
//
// Inside the hq repo the same thing is `node scripts/backstop-install.mjs`,
// which is a shim over this file. Bare words and --flags are both accepted, and
// every message the tool prints names whichever form you actually invoked (see
// SELF below) — telling someone to run a path that does not exist on their
...
// The package root — bin/ and src/ sit under it whether this was installed from
// npm, run with npx out of the cache, or invoked through the repo's shim.
const PKG = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const SRC = path.join(PKG, "src");
// How the user got here, so every "run this to fix it" line is a command that
// exists on their machine. In the hq worktree that is the shim path; anywhere
// else it is the published package.
const SELF = fs.existsSync(path.resolve(PKG, "../../scripts/backstop-install.mjs"))
  ? "node scripts/backstop-install.mjs"
  : "npx @nysgpt/backstop";
...
// `eject` and `--eject` both work. npx users reach for the bare word, the repo
// has always used the flag, and neither should have to know about the other.
const args = process.argv.slice(2).map((a) => a.replace(/^--/, ""));
const mode = ["eject", "doctor", "status", "install"].find((m) => args.includes(m)) ?? "install";
// What to tell the user to type. Bare words are accepted everywhere, so one
// form serves both invocations.
const how = (sub = "") => `${SELF}${sub ? ` ${sub}` : ""}`;
...
if (mode === "doctor") {
  ...
  if (snapshot) {
    ...
    check("daemon runs from snapshot", plistBody.includes(GATEWAY), plistBody.includes(SRC) ? `points at the source tree (${SRC}) — a checkout or reinstall can take the account down` : "not the source tree");
    const drifted = RUNTIME_FILES.filter(
      (f) => sha256(path.join(RUNTIME_DIR, f)) !== sha256(path.join(SRC, f)),
    );
    check("snapshot up to date", drifted.length === 0, drifted.length ? `${drifted.join(", ")} differ from the package — reinstall to pick up fixes` : "matches the package", false);
  }
  ...
  if (problems.length) {
    say(`\n  ${problems.length} broken invariant${problems.length > 1 ? "s" : ""}. Repair:  ${how()}`);
    say(`  Or take backstop out of the path entirely:  ${how("eject")}\n`);
    process.exit(1);
  }
...
// dependency on the package's current state, so editing the source — or a npm
// update, or checking out a branch that predates it — can no longer take a live
// account down. The price is that fixes need a reinstall to land; doctor reports
// the drift.
for (const f of RUNTIME_FILES) {
  fs.copyFileSync(path.join(SRC, f), path.join(RUNTIME_DIR, f));
}
...
//
// NOT SHIPPABLE AS-IS. This reads a Stripe SECRET key and stores it on the
// user's disk, which is fine while the seller and the user are the same person
// (self-hosted, the hq repo, a proving-ground account) and is a key leak the
// moment they are not. A published package selling passes needs the checkout
// session minted server-side by whoever owns the Stripe account; the gateway
// would hold nothing but the returned url. Until that exists, an install with
// no key is a fully working backstop that simply cannot sell — which is the
// correct degradation, not an error.
const stripeKey = (() => {
  if (process.env.STRIPE_SECRET_KEY) return process.env.STRIPE_SECRET_KEY;
  for (const p of [path.join(process.cwd(), ".env.local"), path.resolve(PKG, "../../.env.local")]) {
    try {
      const m = fs.readFileSync(p, "utf8").match(/^STRIPE_SECRET_KEY\s*=\s*["']?([^"'\s]+)/m);
      if (m) return m[1];
    } catch {
      /* next candidate */
    }
...
// The hooks run months after the install, from a shell that knows nothing about
// how backstop got here. Stamp the invocation alongside the runtime so a hook
// firing at 2am names a command that exists on THIS machine.
fs.writeFileSync(path.join(RUNTIME_DIR, "how"), `${SELF}\n`);

fs.writeFileSync(
  STAMP,
  `${JSON.stringify({ installedAt: new Date().toISOString(), fromPackage: PKG, port: PORT }, null, 2)}\n`,
);
...
if (up) {
  ...
  say(`Check it any time with:  ${how("doctor")}\n`);
} else {
