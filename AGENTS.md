...
- **The "panels" menu is `PanelMenu`** (`app/ui/PanelMenu.tsx`) — a boundary chip sitting on the *terminal* `Boundary` trail (after the `terminal.tsx` path chip, before the search icon), opening Activity/Metrics/Console/Compose. Dashed-bordered menu + an info-circle `ButtonChipIcon` on its top-right; closes on outside click. (It used to be a `panels ▾` dropdown inside the terminal header.)
